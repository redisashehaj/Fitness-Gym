...
<?php 
  ...
  if( isset( $_GET[ 'delete' ] ) )
  {
    ...
    $sql_fshi = "DELETE FROM shenimeklient WHERE shenim_id = '$id'"; 
    
      $db->query( $sql_fshi );
    

    header( "Location: http://localhost:1234/palestra/Klient/includes/shenime.php?fshirja=true" );
...
<!-- Forma e shtimit -->
   <form action="http://localhost:1234/palestra/Klient/includes/shenimeQuery.php" method="POST" autocomplete="off">
...
<div class="col-md-6">
        ...
        <tbody>
                <?php while( $shenim = mysqli_fetch_assoc( $pergjigja ) ) : ?>
					<tr>
						<td class="text-center"><a href="http://localhost:1234/palestra/Klient/includes/shenime.php?permbajtja=<?= $shenim[ 'shenim_id' ]; ?>"><?= $shenim[ 'titulli' ]; ?></a></td>
                        <td class="text-center"><?= $shenim[ 'data' ]; ?></td>
                        <td class="text-center"><a href="http://localhost:1234/palestra/Klient/includes/shenime.php?delete=<?= $shenim[ 'shenim_id' ]; ?>" class="btn btn-danger btn-xs"> <i class="glyphicon glyphicon-remove" ></i></a> &nbsp;</td>
                    </tr>
				<?php endwhile; ?>
        </tbody>
</table>
...
<?php	
	if( isset( $_GET[ 'permbajtja' ] ) )
	{
		...
		$sql = "SELECT * FROM shenimeklient WHERE shenim_id = $id";
